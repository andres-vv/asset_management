...
assets = ["IE00B4L5Y983", "MEUD.PA", "CD5.PA", "VUSA.L",
...
for asset in assets:
    ...
    # mydata[asset] = ticker.history(period="max")['Close']
    df = ticker.history(start="2013-07-08", end=today)['Close']
    df.index = df.index.strftime('%Y-%m-%d')
    mydata[asset] = df
    # mydata[a] = wb.DataReader(a, data_source='yahoo',
    #                           start='2015-7-1')['Adj Close']
...
portfolios = pd.DataFrame(
    {'Return': pfolio_returns, 'Volatility': pfolio_volatilities, 'Weights': range(simulations)})
...
portfolios_sorted = portfolios.sort_values(
    by=['Volatility', 'Return'], ascending=[1, 0])
...
print('\n', ' --->     With resp. Return:')
print(str(round(float(
    np.dot(weights_df.loc[[index_weights]], annual_log_returns)*100), 6)) + ' %')
print(
    type(float(np.dot(weights_df.loc[[index_weights]], annual_log_returns)*100)))
...
while x < portfolios_sorted['Volatility'].max():
    ...
    optimal_weights = optimal_weights.append(
        loop_df.loc[loop_df['Return'] == max])
    x = np.round(x + 0.001, 3)

# print(optimal_weights)
optimal_weights.plot(x='Volatility', y='Return',
...
writer = pd.ExcelWriter(
    '/Users/andres/Documents/Beleggen/Markowitz_Optimalization2.xlsx', engine='xlsxwriter')
