...
### IMPORT DATA + BASIC CALCULATIONS
table = pd.DataFrame()
for a in assets:
    table[a] = wb.DataReader(a, data_source='yahoo', start='2018-1-1')['Adj Close']
...
def display_simulated_ef_with_random(mean_returns, cov_matrix, num_portfolios, risk_free_rate):
    ...
    plt.title('Simulated Portfolio Optimization based on Efficient Frontier (GENERATED)')
...
###CALCULATE THE EFFICIENT FRONTIER OF PORTFOLIO
...
def display_calculated_ef_with_random(mean_returns, cov_matrix, num_portfolios, risk_free_rate):
    ...
    plt.title('Calculated Portfolio Optimization based on Efficient Frontier (OPTIMIZED°')
    plt.xlabel('annualised volatility')
    plt.ylabel('annualised returns')
    plt.legend(labelspacing=0.8)
    #plt.show()
...
def efficient_target_return(target):
    ...
    weight = result.x
    result = pd.DataFrame(result.x, index=table.columns, columns=['allocation'])
